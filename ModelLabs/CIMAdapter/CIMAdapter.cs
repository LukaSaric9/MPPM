	...
	public class CIMAdapter
	{
		...
		private bool DoTransformAndLoad(Assembly assembly, ConcreteModel concreteModel, SupportedProfiles extractType, out Delta nmsDelta, out string log)
		{
			...
			try
			{
				...
				switch (extractType)
				{
					case SupportedProfiles.RegulationSchedule:
						{
							// transformation to DMS delta					
							TransformAndLoadReport report = RegulationScheduleImporter.Instance.CreateNMSDelta(concreteModel);

							if (report.Success)
							{
								nmsDelta = RegulationScheduleImporter.Instance.NMSDelta;
								success = true;
							}
							else
							{
                            ...
                            RegulationScheduleImporter.Instance.Reset();
